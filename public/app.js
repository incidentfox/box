...
function vobPipelineMarkup(pipeline) {
  if (!pipeline) return '';
  ...
  return `<section class="vobSection vobPipelineSection"><div class="vobSectionTitle">Decision pipeline <span class="vobMeta">how speech becomes validated evidence</span></div><div class="vobPipelineIntro"><span><b>Version</b> ${esc(pipeline.version || '—')}</span><span>LLM stages emit strict internal envelopes; deterministic stages validate and close the ledger.</span></div><div class="vobPipelineGrid">${stages.map((stage, index) => vobPipelineStageMarkup(stage, index === 0 || index === 1)).join('')}</div></section>`;
}
